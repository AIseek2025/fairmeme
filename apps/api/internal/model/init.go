...
// CloseClickHouse close clickhouse
...
func InitClickHouse() {
	...
	connectString := fmt.Sprintf("tcp://%s:%s?username=%s&password=%s",
		config.Get().ClickHouse.Host, config.Get().ClickHouse.Port,
		config.Get().ClickHouse.User, config.Get().ClickHouse.Pwd)

	// 使用正确的驱动名称 "clickhouse"
	ch, err = sql.Open("clickhouse", connectString)
	if err != nil {
		fmt.Println("InitClickHouse error: " + err.Error())
	}
	return
}
